...
def admin_window():
    ...
    list_ = [var1[0], var2[0], var3[0], var4[0], var5[0], var6[0], var7[0], var8[0], var9[0], var10[0], var11[0]]
    list_app = Listbox(root, selectmode=SINGLE, height=5)
    for i in list_:
    ...
    def circle(in_num):
        for i in range(len(vars_admin)):
            for j in range(len(vars_admin[i])):
                if in_num == vars_admin[i][j]:
                    j += 1
                    in_num = vars_admin[i][j]
                    # print(in_num)
                    return in_num
